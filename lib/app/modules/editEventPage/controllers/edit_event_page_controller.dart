      ...
      if(selectedValue != null && selectedValueRecurring != null){
        try {
          Map<String, dynamic> postAddParameters = {
            ...
            ApiKeyConstants.id:data[ApiKeyConstants.eventId] ?? "",
      ...
      try {
        if (await CommonWidget.internetConnectionCheckerMethod()) {
          ...
          if (getEventDetails != null && getEventDetails?.status != '0') {
            ...
            print(getEventDetails?.result?.title);
            eventText = TextEditingController(text: getEventDetails?.result?.title ?? "");
            print(getEventDetails?.result?.visibleDate);
            visibleDateText = TextEditingController(text: getEventDetails?.result?.visibleDate ?? "");
            print(getEventDetails?.result?.visibleDate);
            dateRangeText = TextEditingController(text: "${getEventDetails?.result?.visibleDate ?? ""} / ${getEventDetails?.result?.visibleDate ?? ""}");
            print(getEventDetails?.result?.dateTime);
            timeText = TextEditingController(text: getEventDetails?.result?.dateTime ?? "");
            print(getEventDetails?.result?.address);
            locationText = TextEditingController(text: getEventDetails?.result?.address ?? "");
            aboutText = TextEditingController(text: getEventDetails?.result?.about ?? "");
            print(getEventDetails?.result?.title);
            // selectedValueRecurring = userModel?.result?.gender ?? "ss" ;
            print(getEventDetails?.result?.title);


            // getEventDetails?.result!.images?.forEach((element) {
            //   if (element != null) {
            //     imageList.add(element as File?);
            //   }
            // });
